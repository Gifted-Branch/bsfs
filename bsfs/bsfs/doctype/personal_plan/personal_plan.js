...
frappe.ui.form.on('Personal Plan', {
	//Get Personal Plan Template items
	template_name: function (frm) {
		if (frm.doc.template_name) {
			frm.clear_table('personal_plan_detail');
			frappe.model.with_doc('Personal Plan Template', frm.doc.template_name, function () {
				let source_doc = frappe.model.get_doc('Personal Plan Template', frm.doc.template_name);
